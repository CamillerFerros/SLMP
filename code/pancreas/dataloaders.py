...
def get_ema_model_and_dataloader(data_root, split_name, batch_size, lr, labelp=10):
    ...
    trainset_lab_a = Pancreas(data_root, split_name, split='train_lab', labelp=labelp)
    lab_loader_a = DataLoader(trainset_lab_a, batch_size=batch_size, shuffle=True, num_workers=0, drop_last=True)

    trainset_lab_b = Pancreas(data_root, split_name, split='train_lab', labelp=labelp, reverse=True)
    lab_loader_b = DataLoader(trainset_lab_b, batch_size=batch_size, shuffle=True, num_workers=0, drop_last=True)

    trainset_unlab_a = Pancreas(data_root, split_name, split='train_unlab', labelp=labelp)
    unlab_loader_a = DataLoader(trainset_unlab_a, batch_size=batch_size, shuffle=True, num_workers=0, drop_last=True)

    trainset_unlab_b = Pancreas(data_root, split_name, split='train_unlab', labelp=labelp, reverse=True)
    unlab_loader_b = DataLoader(trainset_unlab_b, batch_size=batch_size, shuffle=True, num_workers=0, drop_last=True)
