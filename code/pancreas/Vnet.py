...
class VNet(nn.Module):
    ...
    def forward(self, input, need=False, mid=None):

        features = self.encoder(input)
        out, decode_feature = self.decoder(features)
        if need is True:
        ...
        if mid is not None:
            ...
            out_fuse, decode_feature_fuse = self.decoder(feature_fuse)
            return out[0], out_fuse[0]
        return out