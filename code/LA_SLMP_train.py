...
from utils.TPCR_utils import context_mask, mix_loss, parameter_sharing, update_ema_variables
...
parser.add_argument('--exp', type=str,  default='TPCR', help='exp_name')
...
def mean_loss(outputs,label):
    ce = F.cross_entropy(outputs, label)
    dice = DICE_loss(outputs, label)
    return (ce + dice) / 2.0
...
def pre_train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            volume_batch_m,volume_batch_h,label_batch = Augment_3D(volume_batch.cpu(),label_batch.cpu())
            volume_batch_m, volume_batch_h, label_batch = volume_batch_m.cuda(),volume_batch_h.cuda(),label_batch.cuda()

            outputs_m = model(volume_batch_m)
            outputs_h  = model(volume_batch_h)

            loss_ce_m = F.cross_entropy(outputs_m, label_batch)
            loss_dice_m = DICE(outputs_m, label_batch)

            loss_ce_h = F.cross_entropy(outputs_h, label_batch)
            loss_dice_h = DICE(outputs_h, label_batch)

            loss_m = (loss_ce_m + loss_dice_m) / 2
            loss_h = (loss_ce_h + loss_dice_h) / 2

            loss = (loss_m + loss_h * 0.5 ) / 1.5

            iter_num += 1
            writer.add_scalar('pre/loss', loss, iter_num)
            writer.add_scalar('pre/loss_m', loss_m, iter_num)
            writer.add_scalar('pre/loss_h', loss_h, iter_num)

            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            logging.info('iteration %d : loss: %03f, loss_m: %03f, loss_h: %03f'%(iter_num, loss, loss_m, loss_h))
...
def self_train(args, pre_snapshot_path, self_snapshot_path):
    ...
    for epoch in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            mixl_img_m,mixl_img_h,label_l = Augment_3D(mixl_img.cpu(),mixl_lab.cpu())
            mixu_img_m,mixu_img_h,label_u = Augment_3D(mixu_img.cpu(),mixu_lab.cpu())

            mixl_img_m, mixl_img_h, label_l = mixl_img_m.cuda(),mixl_img_h.cuda(),label_l.cuda()
            mixu_img_m, mixu_img_h, label_u = mixu_img_m.cuda(),mixu_img_h.cuda(),label_u.cuda()

            outputs_l_m,l_mid_feature = model(mixl_img_m,need =True)
            outputs_l_h,outputs_l_h_fuse = model(mixl_img_h,mid = l_mid_feature)

            outputs_u_m,u_mid_feature = model(mixu_img_m,need =True)
            outputs_u_h,outputs_u_h_fuse = model(mixu_img_h,mid = u_mid_feature)

            loss_l_m = mean_loss(outputs_l_m,label_l)
            loss_l_h = mean_loss(outputs_l_h,label_l)

            loss_u_m = mean_loss(outputs_u_m,label_u)
            loss_u_h = mean_loss(outputs_u_h,label_u)

            loss_l_fuse = mean_loss(outputs_l_h_fuse,label_l)
            loss_u_fuse = mean_loss(outputs_u_h_fuse,label_u)

            loss_m = (loss_l_m + loss_u_m) / 2.0
            loss_h = (loss_l_h + loss_u_h) / 2.0
            loss_fuse = (loss_l_fuse + loss_u_fuse) / 2.0

            loss = (loss_m + loss_h * 0.25 + loss_fuse * 0.25) / 1.5


            iter_num += 1
            writer.add_scalar('Self/consistency', consistency_weight, iter_num)
            writer.add_scalar('Self/loss_m', loss_m, iter_num)
            writer.add_scalar('Self/loss_h', loss_h, iter_num)
            writer.add_scalar('Self/loss_all', loss, iter_num)

            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            logging.info('iteration %d : loss_all: %03f, loss_m: %03f, loss_h: %03f,loss_fuse: %03f'%(iter_num, loss, loss_m, loss_h,loss_fuse))
...
if __name__ == "__main__":
    ## make logger file
    pre_snapshot_path = "./model/TPCR/LA_{}_{}_labeled/pre_train".format(args.exp, args.labelnum)
    self_snapshot_path = "./model/TPCR/LA_{}_{}_labeled/self_train".format(args.exp, args.labelnum)
    print("Starting TPCR training.")
    for snapshot_path in [pre_snapshot_path, self_snapshot_path]:
        if not os.path.exists(snapshot_path):
            os.makedirs(snapshot_path)
        if os.path.exists(snapshot_path + '/code'):
            shutil.rmtree(snapshot_path + '/code')
    shutil.copy('../code/LA_TPCR_train.py', self_snapshot_path)
    # -- Pre-Training
