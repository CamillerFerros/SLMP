...
class LAHeart(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_01 = (image - np.min(image)) / (np.max(image) - np.min(image))
        image_01 = image_01.astype(np.float32)
        # label_01 = (label - np.min(label)) / (np.max(label) - np.min(label))
        # label_01 = label_01.astype(np.float32)

        sample = {'image': image_01, 'label': label}
        if self.transform:
            sample = self.transform(sample)
...
class LabeledBatchSampler(Sampler):
    """Iterate through primary indices (labeled indices)"""
    ...
    def __iter__(self):
        ...
        return (
            primary_batch
            for primary_batch in grouper(primary_iter, self.batch_size)
        )
