...
class Encoder(nn.Module):
    ...
    def forward(self, input):
        ...
        res = [x1, x2, x3, x4, x5,x5_nodropout]
        return res
...
class Decoder(nn.Module):
    ...
    def forward(self, features):
        ...
        res = [x5_up,x6_up,x7_up,x8_up,x9]
...
class VNet(nn.Module):
    ...
    def forward(self, input,mid = None,need=False):
        features = self.encoder(input)
        out_seg, decode_features = self.decoder(features)
        if need is True:
        ...
        if mid is not None:
            ...
            for i in range(len(features)):
                feat1 = features[i]
                feat2 = mid[i]
                b,c,h,w,d = feat1.shape
                lamda = 0.5
                mask = torch.bernoulli(torch.full((b,c,h,w,d),lamda)).cuda()
                fuse = feat1 * mask + feat2 * (1 - mask)
                feature_fuse.append(fuse)
            output_fuse, decode_features_fuse = self.decoder(feature_fuse)
            return out_seg,output_fuse
