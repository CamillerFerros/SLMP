...
class Decoder(nn.Module):
    ...
    def forward(self, feature):
        ...
        x6 = self.up2(x5, x2)
        x7 = self.up3(x6, x1)
        x_last = self.up4(x7, x0)
        res = [x4,x5,x6,x7,x_last]
...
class UNet_2d(nn.Module):
    ...
    def forward(self, x,mid=None,need=False):
        feature = self.encoder(x)
        output, features = self.decoder(feature)
        if mid is not None:
            feature_fuse = []
            for i in range(len(feature)):
                ...
                b,c,h,w = feat1.shape
                lamda = 0.5
                mask = torch.bernoulli(torch.full((b,c,h,w),lamda)).cuda()
                fuse = feat1 * mask + feat2 * (1 - mask)
                feature_fuse.append(fuse)
            output_fuse, features_fuse = self.decoder(feature_fuse)
            return output,output_fuse
        if need is True:
